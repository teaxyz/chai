...
class TestDedupe:
    ...
    def test_case_2d_new_canon_new_mapping(
        ...
        assert created_canon.url_id == ids["url1"], "Canon should reference correct URL ID"
    ...
    def test_case_2d_new_canon_update_mapping(
        ...
        assert updated_mapping["canon_id"] == created_canon.id, "Should update to NEW canon"
        assert updated_mapping["canon_id"] != ids["canon2"], "Should NOT point to old canon"
    ...
    def test_case_2b_update_existing_mapping(
        ...
        mock_db.get_current_canon_packages.return_value = {
            ids["pkg1"]: {"id": uuid4(), "canon_id": ids["canon2"]}  # Linked to wrong canon
        }
        mock_db.get_packages_with_homepages.return_value = [(package, homepage_url)]
        ...
        updated_mapping = updated_canon_packages[0]
        assert "id" in updated_mapping, "Update should include canon package ID"
        assert updated_mapping["canon_id"] == ids["canon1"], "Should update to correct canon"
        assert updated_mapping["canon_id"] != ids["canon2"], "Should NOT point to wrong canon"
        assert "updated_at" in updated_mapping, "Update should include timestamp"
    ...
    def test_multiple_packages_same_homepage_creates_single_canon(
        ...
        assert canon_ids.pop() == created_canon.id, "Both should map to the created canon"
